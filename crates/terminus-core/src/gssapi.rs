...
//!
//! Ported from production `main` (#113). The authenticator is generic over the
//! russh client handler so rio's [`crate::ssh::ClientHandler`] can reuse it.
...
/// Authenticate `session` as `host.username` via Kerberos GSSAPI.
pub async fn authenticate<H>(session: &mut Handle<H>, host: &Host) -> Result<bool>
where
    H: Handler + Send,
            ...
            if let Some(mech) = selected_mechanism.as_deref() {
                if !Self::offered_mech(mech) {
                    return Err(Error::msg(
                        "SSH server selected an unsupported GSSAPI mechanism",
                    ));
                }
            }
            if self.ctx.is_none() {
